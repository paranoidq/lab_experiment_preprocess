...
stop_words_path = dir_path + 'stopwords'
enhanced_stops_path = dir_path + 'en_stops/' + 'en_stops_90'
id2wordspath = dir_path + 'words'

feat_src_path = dir_path + 'features_src'
feat_path = dir_path + 'features'

edges_path = dir_path + 'edges'

pats_path = dir_path + 'patterns'
pats_repr_path = dir_path + 'patterns_repr'
...
def process():
    ...
    stops = loader.load_stop_words(stop_words_path)
    en_stops = loader.load_enhanced_stops(enhanced_stops_path)
    stops = stops.union(en_stops)

    # 过滤stop words
    id2words = loader.load_id2words(id2wordspath)
    filtered_words = dict()
    ...
    uid2feats = dict()
    with open(feat_src_path, 'r', encoding='utf-8') as fin:
    ...
    # 生成trans, 并写入文件
    edges = loader.load_edges(edges_path)
    trans_path = dir_path + 'trans'
    with open(trans_path, 'w+', encoding='utf-8') as fout:
        for u1, u2 in edges:
            trans = set()
    ...
    os.system('/Users/paranoidq/316-data/polblogs/fpgrowth -tm -q1 -m2 -s70 -v\"|%a\" ' +
              trans_path + ' ' + pats_path)
    with open(pats_path, 'r', encoding='utf-8') as fin:
        with open(pats_repr_path, 'w+', encoding='utf-8') as fout:
            for line in fin:
                sp = line.strip('\n').split('|')
                support = sp[-1]
                items = sp[0].split(' ')
